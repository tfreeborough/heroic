...
export const ABILITY_CODEX: Record<AbilityId, { hint: string; quote: string; desc: string; chips: CodexChip[] }> = {
  ...
  "true-ice": {
    hint: "entomb the nearest enemy — frozen, and untouchable",
    quote: "True ice does not melt, and it does not bargain. Three heartbeats belong to you.",
    desc: "Hurls a shard at the nearest enemy in range and seals them in ice: they can't move, aim or cast while it holds — and nothing touches them either, every blow reading IMMUNE off the block. The fight simply loses a body for the duration: heal, reposition, or turn on their friend. Freezing the same body again grants shorter and shorter tombs until the round resets. No mark in range, no cast.",
    chips: [
      { label: "FREEZE", value: `${TRUE_ICE.freezeSeconds}s` },
      { label: "RANGE", value: `${TRUE_ICE.maxRange}px` },
      { label: "REPEAT", value: `×${TRUE_ICE.diminishFactor} each, per round` },
    ],
  },
  "magic-mirror": {
    hint: "swap places with the furthest enemy — after a loud warning",
    quote: "The glass shows you standing somewhere better. Then it insists.",
    desc: "Marks the enemy furthest from you and, one loudly-telegraphed second later, trades places with them. The warning is theirs to spend — a dash or planted feet at the crossing instant refuses the trip. Dark for the opening seconds of every round: the mirror answers a fight, it doesn't open one.",
    chips: [
      { label: "DELAY", value: `${MAGIC_MIRROR.delaySeconds}s warning` },
      { label: "TARGET", value: "furthest enemy" },
      { label: "ROUND START", value: `dark for ${MAGIC_MIRROR.lockoutSeconds}s` },
    ],
  },
  "elven-cloak": {
    hint: "fade from sight — your deeds stay visible, your body doesn't",
    quote: "Woven from the hour after sunset. The crowd sees your arrows land, and nothing loosing them.",
    desc: "Wraps you in woven dusk: your body all but vanishes from enemy screens and their aim can't find you — locks break, and no new mark lands while you're faded. Everything you DO stays plain to see: every shot, every cast flash, every sound. Allies (and you) keep a faint ghost to fight beside.",
    chips: [
      { label: "DURATION", value: `${ELVEN_CLOAK.duration}s` },
      { label: "HIDES", value: "the body, not the deeds" },
      { label: "ENEMY AIM", value: "no locks in or held" },
    ],
  },
};
