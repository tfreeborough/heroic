...
export type StatusKind = "slow" | "bleed" | "poison" | "taunt" | "mirror-guard";
...
const timeLeft = (p: PlayerSnapshot, kind: StatusKind): number => {
  ...
  if (kind === "poison") return p.poisonLeft;
  if (kind === "taunt") return p.tauntLeft;
  return p.abilities.find((s) => s.id === kind)?.active ?? 0;
};

const KINDS: readonly StatusKind[] = ["slow", "bleed", "poison", "taunt", ...RING_ABILITIES];
