...
const runAsync = (tool: "ffmpeg" | "ffprobe", args: string[]) => exec(command(tool, args), CWD);

export const ffprobe = (file: string): { seconds: number; width: number; height: number; fps: number } => {
  const r = run("ffprobe", ["-v", "error", "-select_streams", "v:0", "-show_entries", "stream=width,height,r_frame_rate:format=duration", "-of", "json", file]);
  if (!r.ok) throw new Error(`ffprobe failed for ${basename(file)}: ${r.err.trim()}`);
  const json = JSON.parse(r.out.slice(r.out.indexOf("{"))) as { streams: { width: number; height: number; r_frame_rate: string }[]; format: { duration: string } };
...
export const cleanupSeconds = (spec: CleanupSpec): number => {
  ...
  return spec.transition === "cut" ? total : Math.max(0.1, total - joins * spec.transitionSeconds);
};

/**
 * Cut + crop a recording into a new file (H.264, faststart). Several kept
 * segments are joined in order — a hard cut (concat), a crossfade (xfade
 * "fade") or a dip to black (xfade "fadeblack"). Cropping is by fractions of
 ...
 */
export const cleanupClip = async (input: string, output: string, spec: CleanupSpec) => {
  ...
  const parts: string[] = [];
  segs.forEach((s, i) => {
    parts.push(`[0:v]trim=start=${s.start.toFixed(3)}:end=${s.end.toFixed(3)},setpts=PTS-STARTPTS,${crop},format=yuv420p[v${i}]`);
  ...
  if (segs.length > 1) {
    if (spec.transition === "cut") {
      const ins = segs.map((_, i) => `[v${i}]${audio ? `[a${i}]` : ""}`).join("");
      parts.push(`${ins}concat=n=${segs.length}:v=1:a=${audio ? 1 : 0}[vout]${audio ? "[aout]" : ""}`);
      vOut = "[vout]";
      aOut = "[aout]";
    } else {
      // Each join overlaps the last `d` seconds of what's built so far with the next segment.
      const d = Math.max(0.1, Math.min(spec.transitionSeconds, ...segs.map((s) => (s.end - s.start) / 2)));
      const kind = spec.transition === "dip" ? "fadeblack" : "fade";
      let built = segs[0]!.end - segs[0]!.start;
      for (let i = 1; i < segs.length; i++) {
        const offset = Math.max(0, built - d);
        parts.push(`${vOut}[v${i}]xfade=transition=${kind}:duration=${d.toFixed(3)}:offset=${offset.toFixed(3)}[vx${i}]`);
